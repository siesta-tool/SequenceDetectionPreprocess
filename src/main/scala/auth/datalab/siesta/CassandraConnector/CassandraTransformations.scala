 ...
 */
object CassandraTransformations {
  ...
  def transformSeqToRDD(df: DataFrame): RDD[Sequence] = {
    df.rdd.map(x => {
      val sequence_id = x.getAs[String]("sequence_id")
      val pEvents = x.getAs[Seq[String]]("events").zipWithIndex.map { case (e, idx) =>
        val splitted = e.split(",")
        new Event(trace_id = sequence_id, event_type = splitted(0), timestamp = splitted(1), position = idx)
      }
      new Sequence(pEvents.toList, sequence_id)
    })
  }
   ...
   */
  case class CassandraSequence(events: List[String], sequence_id: String)
   ...
   */
  def transformSeqToWrite(data: RDD[EventTrait]): RDD[CassandraSequence] = {
    data.groupBy(_.trace_id).map { case (trace_id, events) =>
      val s = events.toList.sortWith((a, b) => a.position < b.position)
      val formattedEvents = s.map(e => s"${e.event_type},${e.timestamp}")
      CassandraSequence(formattedEvents, trace_id)
    }
  }
   ...
   */
  case class CassandraSingle(event_type: String, trace_id: String, occurrences: List[String])
   ...
   */
  def transformSingleToRDD(df: DataFrame): RDD[Event] = {
    df.rdd.flatMap(row => {
      val event_name = row.getAs[String]("event_type")
      val trace_id = row.getAs[String]("trace_id")
      row.getAs[Seq[String]]("occurrences").map(oc => {
        val s = oc.split(",")
        new Event(trace_id= trace_id, event_type = event_name, position= s(0).toInt, timestamp = s(1))
      })
    })
  }
   ...
   */
  def transformSingleToWrite(singleRDD: RDD[EventTrait]): RDD[CassandraSingle] = {
    singleRDD
      .groupBy(x => (x.event_type, x.trace_id))
      .map(x => {
        val occurrences: List[String] = x._2.map(event => s"${event.position},${event.timestamp}").toList
   ...
   */
  case class CassandraLastChecked(event_a: String, event_b: String, trace_id: String, timestamp: String)
  ...
  def transformLastCheckedToRDD(df: DataFrame): RDD[Structs.LastChecked] = {
    df.rdd.map(r => {
      ...
      val trace_id = r.getAs[String]("trace_id")
   ...
   * occurrence is a string " trace,,id,, || timestampA,,1,,|timestampB,,1,, , timestampA,,2,,|timestampB,,2,, "
        ...
        s_1(1).split(",").map(o => {
          if (bc.value == "positions") {
            ...
            Structs.PairFull(eventA = eventA, eventB = eventB, id = id, timeA = null,
              timeB = null, positionA = s_2(0).toInt, positionB = s_2(1).toInt)
          } else {
            val s_2 = o.split("\\|")
            Structs.PairFull(eventA = eventA, eventB = eventB, id = id, timeA = Timestamp.valueOf(s_2(0)),
              timeB = Timestamp.valueOf(s_2(1)), positionA = -1, positionB = -1)
          }
        })
      })
    })
