...
import auth.datalab.siesta.TraceGenerator.{TraceGenerator, TraceGeneratorDepricated}
import org.apache.spark.rdd.RDD

/**
 * Creates the RDD containing the traces either based on an input file or based on randomly generated traces
 */
object IngestingProcessDepricated {
  ...
  def getData(c:Config): RDD[Structs.Sequence] ={
    if (c.filename != "synthetic") {
      ReadLogFileDepricated.readLog(c.filename)
    } else {
      val traceGenerator = new TraceGeneratorDepricated(c.traces, c.event_types, c.length_min, c.length_max)
