...
object StreamingProcessAttributes {


  case class CustomState(number_of_events: Int, events: mutable.HashMap[String, Array[(Timestamp, Int)]]) extends Serializable

  case class CustomStateAttributes(number_of_events: Int, events: mutable.HashMap[String, Array[(Timestamp, Int, Map[String,String])]]) extends Serializable
  ...
  def createPairAttributes(eva:(String,Timestamp,Int,Map[String,String]),evb:(EventStream,Int)):Structs.StreamingPairAttributes={
    val ts_b=Timestamp.valueOf(evb._1.timestamp)
    Structs.StreamingPairAttributes(eva._1, evb._1.event_type, evb._1.trace,eva._2, ts_b, eva._3, evb._2, eva._4, evb._1.attributes)
  }

  def createPairAttributes(eva:(String,Timestamp,Int,Map[String,String]),evb:(String,Timestamp,Int,Map[String,String]),traceId:String):Structs.StreamingPairAttributes={
    Structs.StreamingPairAttributes(eva._1, evb._1, traceId,eva._2, evb._2, eva._3, evb._3, eva._4, evb._4)
  }
   ...
   */
  def calculatePairs(traceId: String, eventStream: Iterator[EventStream], groupState: GroupState[CustomStateAttributes],lookback:Int): Iterator[Structs.StreamingPairAttributes] = {

    //TODO: handle lookback, mode (positions/timestamps)


    val values = eventStream.toSeq
    val initialState = new CustomStateAttributes(0, new mutable.HashMap[String, Array[(Timestamp, Int, Map[String,String])]])
    ...
    val generatedPairs = mutable.ArrayBuffer[Structs.StreamingPairAttributes]()
    ...
    for (evb <- newEventsWithIndex) {
      ...
      val same_type_events: Array[(Timestamp, Int, Map[String, String])] = oldState.events.getOrElse(evb._1.event_type, Array.empty[(Timestamp, Int, Map[String, String])])
      var prev_event: (String, Timestamp, Int, Map[String, String]) = null
      if (same_type_events.isEmpty) {
        oldState.events(evb._1.event_type) = Array((Timestamp.valueOf(evb._1.timestamp), evb._2, evb._1.attributes))
      } else {
        ...
        if (same_type_events.length == 1) { //there is only one event -> append the new one
          ...
          prev_event = (evb._1.event_type, same_type_events.head._1, same_type_events.head._2, same_type_events.head._3)
          oldState.events(evb._1.event_type) = oldState.events(evb._1.event_type) :+ (Timestamp.valueOf(evb._1.timestamp), evb._2, evb._1.attributes)
        } else { //there are two events, replace the second one
          prev_event = (evb._1.event_type, same_type_events(1)._1, same_type_events(1)._2, same_type_events(1)._3)
          val ts_b: Timestamp = Timestamp.valueOf(evb._1.timestamp)
          oldState.events(evb._1.event_type)(1) = (ts_b, evb._2, evb._1.attributes)
        }
        if (ts_b.getTime - prev_event._2.getTime <= lookbackMillis) {
          val event = StreamingProcessAttributes.createPairAttributes(prev_event, evb)
      ...
      val evb_transformed = (evb._1.event_type, Timestamp.valueOf(evb._1.timestamp), evb._2, evb._1.attributes)
        ...
        for (key <- keys) {
          if (key != evb._1.event_type) { //for all the other keys
            ...
            while (i < oldState.events(key).length){
              ...
              val eva_transformed = (key, eva._1, eva._2, eva._3)
              val diff = evb_transformed._2.getTime - eva_transformed._2.getTime
              if (diff >= 0 && diff < lookbackMillis){
                generatedPairs.append(createPairAttributes(eva_transformed, evb_transformed, traceId.toString))
    ...
    val newState = new CustomStateAttributes(oldState.number_of_events + values.size, oldState.events)
