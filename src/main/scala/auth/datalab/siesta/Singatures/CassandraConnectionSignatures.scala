...
class CassandraConnectionSignatures extends Serializable {
  ...
  // Using default values for ScyllaDB compatibility
  private var _configuration: SparkConf = _
  private val cassandra_replication_class: String = "SimpleStrategy"
  private val cassandra_replication_factor: String = "1"
  ...
  def startSpark(): Unit = {
    ...
    Logger.getLogger("Spark Config").log(Level.INFO, s"Connecting to ScyllaDB at $cassandra_host:$cassandra_port with user $cassandra_user")

    val spark = SparkSession.builder()
      .config(_configuration)
      .getOrCreate()

    // Verify connection parameters are being used
    spark.sparkContext.getConf.getAll.filter(_._1.contains("cassandra")).foreach { case (key, value) =>
      Logger.getLogger("Cassandra Config").log(Level.INFO, s"$key = $value")
    }
    ...
    try {
      CassandraConnector(spark.sparkContext.getConf).withSessionDo { session =>
        session.execute("create keyspace if not exists " + cassandra_keyspace_name + " WITH replication = "
          + "{'class':'" + cassandra_replication_class + "', 'replication_factor':" + cassandra_replication_factor + "}")
