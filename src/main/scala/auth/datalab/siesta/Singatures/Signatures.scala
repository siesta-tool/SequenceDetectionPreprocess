...
import auth.datalab.siesta.BusinessLogic.IngestData.IngestingProcessDepricated
...
object Signatures {
  ...
  def execute(c: Config): Unit = {
    ...
    try {
      ...
      val sequenceRDD: RDD[Structs.Sequence] = IngestingProcessDepricated.getData(c)
